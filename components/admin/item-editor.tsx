  ...
  function saveAll() {
    ...
    start(async () => {
      for (const f of dirty) {
        const res = await updateItemField({ table, id, field: f.name, value: values[f.name] ?? "" });
        if (!res.ok) {
          toast.error(res.error);
          return;
        }
      }
      toast.success(dirty.length === 1 ? "Campo salvo." : `${dirty.length} campos salvos.`);
      router.refresh();
    });
  }
  ...
  return (
    <div className="rounded-lg border bg-card p-4">
      ...
      <div className="mt-3 flex items-center gap-3">
        <Button size="sm" onClick={saveAll} disabled={pending || dirty.length === 0}>
          Salvar alterações
        </Button>
        {dirty.length > 0 && (
          <span className="text-xs text-muted-foreground">
            {dirty.length} {dirty.length === 1 ? "alteração não salva" : "alterações não salvas"}
          </span>
        )}
      </div>
    </div>
