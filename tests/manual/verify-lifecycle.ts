...
import { contentEditions, rawSignals, ingestionSources, platforms } from "@/db/schema";
...
const [after] = await db.select().from(contentEditions).where(eq(contentEditions.id, expired!.id));
check("edição expirada foi arquivada", after?.isArchived === true && after?.status === "archived");

const remaining = await db.select().from(rawSignals).where(eq(rawSignals.id, oldSignal!.id));
...
check("enumeração de assinantes do digest funciona (>=0)", Array.isArray(emails));

// limpeza do fixture
await db.delete(contentEditions).where(and(eq(contentEditions.id, expired!.id)));
