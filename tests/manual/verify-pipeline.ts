...
await db
  ...
  .where(
    and(
      eq(contentEditions.platformId, ig.id),
      eq(contentEditions.editionDate, date1),
    ),
  );
await db
  .delete(contentEditions)
  .where(
    and(
      eq(contentEditions.platformId, ig.id),
      eq(contentEditions.editionDate, date2),
    ),
  );

// 1) Geração válida → draft
const r1 = await runDailyGeneration({
  platformId: ig.id,
...
const [ed] = await db
  ...
  .where(eq(contentEditions.id, r1.editionId!));
...
const trends = await db
  .select()
  .from(trendItems)
  .where(eq(trendItems.editionId, r1.editionId!));
const heads = await db
  .select()
  .from(headlinesTable)
  .where(eq(headlinesTable.editionId, r1.editionId!));
...
const [run1] = await db
  .select()
  .from(generationRuns)
  .where(eq(generationRuns.id, r1.runId));
check("generation_run completed", run1?.status === "completed");

// 2) Idempotência
const r2 = await runDailyGeneration({
  platformId: ig.id,
  platformSlug: "instagram",
  editionDate: date1,
});
check(
  "run2 idempotente (skipped)",
  r2.skipped === true && r2.editionId === r1.editionId,
);

// 3) Saída inválida → NÃO grava + run failed
process.env.PIPELINE_FORCE_INVALID = "headlines";
const r3 = await runDailyGeneration({
  platformId: ig.id,
  platformSlug: "instagram",
  editionDate: date2,
});
check("run3 falhou (saída inválida)", r3.ok === false);
const edits2 = await db
  .select()
  .from(contentEditions)
  .where(
    and(
...
const [run3] = await db
  .select()
  .from(generationRuns)
  .where(eq(generationRuns.id, r3.runId));
check(
  "generation_run3 = failed com erro",
  run3?.status === "failed" && !!run3?.errorMessage,
);
...
check(
  "run4 estourou teto de custo e não publicou",
  r4.ok === false && /custo/i.test(r4.error ?? ""),
);
