...
function badge(label, bg, fg, brd) {
  ...
  if (brd) {
    b.strokes = [{ type: "SOLID", color: brd }];
    b.strokeWeight = 1;
  }
...
function tab(label, active) {
  ...
  t.paddingBottom = 6;
  t.cornerRadius = 8;
  t.fills = active ? [{ type: "SOLID", color: FOREST100 }] : [];
  t.appendChild(
    txt(
      label,
      "Inter",
...
top.paddingLeft = 24;
top.paddingRight = 24;
top.fills = [{ type: "SOLID", color: WHITE }];
top.strokes = [{ type: "SOLID", color: BORDER }];
...
top.resize(top.width, 64);
top.counterAxisSizingMode = "FIXED";
const brandWrap = figma.createAutoLayout("HORIZONTAL");
brandWrap.itemSpacing = 10;
brandWrap.counterAxisAlignItems = "CENTER";
brandWrap.fills = [];
brandWrap.appendChild(
  txt("Admin · Inteligência Criativa", "Newsreader", "Medium", 20, INK, {
    ls: -0.3,
  }),
);
brandWrap.appendChild(badge("superadmin", MINT50, INK, BORDER));
top.appendChild(brandWrap);
const topRight = figma.createAutoLayout("HORIZONTAL");
topRight.itemSpacing = 14;
topRight.counterAxisAlignItems = "CENTER";
topRight.fills = [];
topRight.appendChild(txt("Voltar ao app", "Inter", "Medium", 14, MUTED));
const sair = figma.createAutoLayout("HORIZONTAL");
sair.paddingLeft = 14;
sair.paddingRight = 14;
sair.primaryAxisAlignItems = "CENTER";
sair.counterAxisAlignItems = "CENTER";
sair.cornerRadius = 8;
sair.fills = [];
sair.strokes = [{ type: "SOLID", color: BORDER }];
sair.strokeWeight = 1;
sair.appendChild(txt("Sair", "Inter", "Medium", 14, INK));
sair.resize(70, 36);
sair.primaryAxisSizingMode = "AUTO";
sair.counterAxisSizingMode = "FIXED";
...
// Nav tabs
const tabs = figma.createAutoLayout("HORIZONTAL");
tabs.itemSpacing = 4;
tabs.fills = [];
tabs.paddingBottom = 8;
tabs.strokes = [{ type: "SOLID", color: BORDER }];
tabs.strokeBottomWeight = 1;
tabs.strokeTopWeight = 0;
tabs.strokeLeftWeight = 0;
tabs.strokeRightWeight = 0;
...
head.appendChild(
  txt("Fila de revisão", "Newsreader", "Medium", 34, INK, { ls: -0.6 }),
);
head.appendChild(
  txt(
    "Único passo humano: nada publica sem aprovação.",
    "Inter",
    "Regular",
    16,
    MUTED,
...
function row(title, date, b) {
  ...
  r.strokes = [{ type: "SOLID", color: BORDER }];
  r.strokeWeight = 1;
...
row(
  "Inteligência Criativa — Instagram — 2026-06-13",
  "Gerada pelo pipeline · aguardando revisão",
  badge("Pendente", BLUSH100, ROSE900),
);
row(
  "Inteligência Criativa — Instagram — 2026-06-12",
  "Em revisão por você",
  badge("Em revisão", MINT50, INK, BORDER),
);
row(
  "Inteligência Criativa — Instagram — 2026-06-11",
  "Rejeitada · motivo registrado",
  badge("Rejeitada", ROSE50, ROSE900, { r: 0.937, g: 0.847, b: 0.851 }),
);
